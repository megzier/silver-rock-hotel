/* Silver Rock Boutique Resort — Rooms & Facilities interactions V1.11 */
...
function selectFacility(index, options = {}) {
    ...
    if (!stage || !hero || !title || !desc || !kicker || !meta || !link || !progress) return;
    ...
    if (current) current.textContent = formatFacilityNumber(activeLuxuryFacility);
    ...
    document.querySelectorAll('[data-facility-button]').forEach((button, buttonIndex) => {
        ...
        // Never call scrollIntoView here. The facilities slideshow rotates automatically,
        // and scrollIntoView was pulling visitors away from whichever section they were reading.
        // The active facility tab is still highlighted without changing the page position.
    });
...
function restartLuxuryFacilityAuto() {
    ...
    luxuryFacilityTimer = window.setInterval(() => {
        selectFacility(activeLuxuryFacility + 1, { fromAuto: true });
    }, 8000);
}
